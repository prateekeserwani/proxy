...
def backprop_receptive_field(arch_str,image, predicted_class, scoremap, use_max_activation=True):
    model = nasbench2.get_model_from_arch_str(arch_str, number_of_classes)
    model.to('cpu')
    ...
    grad = torch.ones_like(out, requires_grad=True)
    temp = torch.zeros_like(out) 
    temp = (temp[0, predicted_class] + 1 )*scoremap
    if not use_max_activation:
        #print(scoremap,scoremap.shape,grad.shape)
        grad = grad*temp
    else:
...
def find_rect(activations):
    ...
    print('rectangular bounding box', rect)
...
def run_resnet_inference(original_image, arch_str):
    ...
    image = cv2.resize(cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB),(32,32))
    ...
    receptive_field_net_pred_map = backprop_receptive_field(arch_str,image, scoremap=score_map, predicted_class=predicted_class, use_max_activation=False)
    ...
    visualize_activations(original_image, receptive_field_net_pred_map, show_bounding_rect=True)
...
def main():
    # Read the image
    image_path = './receptive_field/camel.jpg'
    image = cv2.imread(image_path)
    for i, arch_str in enumerate(api):
        print(arch_str)
        run_resnet_inference(image, arch_str)
